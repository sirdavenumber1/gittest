...
def getLog(api_core, podName):
   try:
      ...
      print("Exception listing log for pod %s namespace %s error: %s\n" % (podName, kNameSpace, e))
...
def status(api_batch, kJobname, pod_ok):
   jobStatus = "Success"
   jobRunning = "Running"
   podLabelSelector = 'job-name=' + kJobname
   ...
   if pod_ok:
      ...
      streamLog(core_client, podName)                # Stream the log output
      ...
      print("Job %s failed - pod %s is not ok\n" % (kJobname, podName))
...
def streamLog(api_core, podName):
   print("Streaming log output from Pod: " + podName)
   w = watch.Watch()
...
def usage():
   ...
   print("\t-k, --commands\t\tcommands to run (default None, continer entrypoint will be used)")
   ...
   print("\t-s, --imagepullsecret\ttname of image_pull_secret")
...
def used_opts(kJobname, kYaml, kVname,
   ...
   print("\tjobname: \t\t\t%s \n"
         ...
         % (kJobname, kVname, kVvalue, kImagename, kimagepullpolicy,
            ...
            kvolpath, kpvolclaim, kYaml, kcommands, kargs, kNameSpace))
...
def main(argv):
   ...
   #   k|commands          Commands to execute by container
   #   m|volname           Volume mount name
   #   n|namespace         Kubernetes Name Space
   #   p|imagepullpolicy   Always or Latest
   #   r|restartpolicy     default is Never
   #   s|imagepullsecret   name of image_pull_secret
   ...
   #
   try:
      opts, arg = getopt.getopt(argv,"ha:b:c:e:H:i:j:k:m:n:p:r:s:t:v:y:",
            ["args=",
             "backofflimit=", 
             "claim=",
             "envname=", 
...
#  config.load_kube_config()                      # Out of cluster 
   ...
   if kYaml != '':
      ...
      used_opts(kJobname, kYaml, kVname, kVvalue, kImagename, kimagepullpolicy, kimagepullsecret, krestartpolicy,
             kbackofflimit, khostpath, kvolname, kvolpath,kpvolclaim, kcommands, kargs)
      job = create_job_object(kJobname, kImagename, kVname, kVvalue, kimagepullpolicy, kimagepullsecret, krestartpolicy, kbackofflimit, khostpath, kvolname, kvolpath, kpvolclaim, kcommands, kargs)
      try:
   ...
   while True:
      ...
      pod_ok = readpod(core_client, podName)
      if not pod_ok:
         time.sleep(1)
         #print("Waiting for pod status")
      else:
         break
      if elapsed_time > seconds:
         print("Pod failed to initialize: " + str(int(elapsed_time)) + " seconds")
         break

   jobStatus, podsActive, podsSucceeded, podsFailed = status(batch_client, kJobname, pod_ok)
